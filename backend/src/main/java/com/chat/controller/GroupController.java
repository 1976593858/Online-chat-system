...
@Tag(name = "群聊管理接口", description = "创建群聊、加入/退出、群详情、群成员、群消息")
public class GroupController {
    ...
    @Operation(summary = "退出群聊", description = "退出群聊，若群内无成员则自动解散")
    ...
    @Operation(summary = "分页查询群历史消息", description = "按时间倒序分页返回群聊天记录")
    public Result<PageResult<GroupMessageVO>> messages(
            @Parameter(description = "群ID") @PathVariable Long groupId,
            @Valid @ParameterObject PageQuery pageQuery) {
        return Result.success(groupService.getGroupMessages(groupId, pageQuery.getPageNo(), pageQuery.getPageSize()));
    }

    @GetMapping("/{groupId}/export")
    @Operation(summary = "导出群聊天记录", description = "返回 txt 文件字节流")
    public ResponseEntity<byte[]> export(@Parameter(description = "群ID") @PathVariable Long groupId) {
        byte[] body = groupService.exportGroupMessages(groupId);
